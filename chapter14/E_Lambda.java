...
: 메서드
    ...
    - and(Predicate other), or(Predicate other), negate() : Predicate 조항을 위해 사용
    cf) negate : 부정하다 (현재의 결과를 반대로 반환)

2. Fuction
: 입력 값을 받아 특정 연산을 수행한 후 결과를 반환
: 메서드
    - R apply(T t) : 입력 값을 받아 반환한 값을 반환
    - andThen(Fuction after) : 현재 결과를 다른 Fuction의 입력으로 연결
    - compose(Fuction before) : 다른 Fuction의 결과를 현재 입력으로 연결
