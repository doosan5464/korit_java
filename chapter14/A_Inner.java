...
/*
내부 클래스(중첩클래스, Inner Class, Nested Class)
...
3. 메서드 내부 클래스 (지역 클래스)
    : 메서드 내에 정의된 내부 클래스
        >> 해당 메서드 내에서만 사용 가능
        >> 메서드의 지역 변수에만 접근 O
        cf) 로컬 변수 (외부 클래스 멤버 변수)가 final 인 경우에는 접근 O
...
class OuterClass {
    ...
    static class StaticClass {
        void diplay() {
            // System.out.println("외부 클래스 필드에 접근 : " + outerField); 정적 필드가 아니라 안됨
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        innerClass.diplay();    // 외부 클래스 필드에 접근 : 외부 클래스의 필드
        ...
        staticClass.diplay();   // 외부 클래스 정적 필드에 접근 : 외부 클래스의 정적 필드
        ...
        abstractClass.display();    // 익명 내부 클래스
        ...
        System.out.println(abstractClass == abstractClass2); // false
