...
public class A_Array {
    public static void main(String[] args) {
        ...
        // 배열
        int[] array; // 정수형 데이터를 담을 배열

        // === 배열의 생성 ===
        // 실제 데이터가 저장 X, 데이터 배열이 저장된 첫 번째 메모리주소(배열의 첫번째인덱스 기준으로 고유주소)가 저장
        // >> new 데이터타입[배열의 길이]
        // 배열 생성시 모든 요소는 해당 데이터 값의 기본 데이터로 채워진다
        // 문자열(null), 숫자(0), 논리형(false)
        array = new int[3];
...
//        System.out.println(array[3]); // 에러발생


        // 1) 데이터타입[] 타입명 = new 데이터타입[] {요소1, 요소2, ...}
        ...
        int orinalLength = original.length;
        for(int i = 0; i < orinalLength; i++){
            if(i==removeIndex){
