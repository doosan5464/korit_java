...
interface PlayingCard {
    // 1. 멤버 변수
    // : 상수 선언
    // : public(어디서나) static(정적. 다른 곳이라도 변하지 않음) final(변할 수 없음. 재할당 불가) 특성을 가짐
    public static final int SPADE = 4;  // 상수는 선언과 동시에 초기화 해야함
    public static int DIAMOND = 3;
    public int HEART = 2;
    int CLOVER = 1; // 생략이라 4개 다 똑같음
    ...
    String getCardKind();   // 생략이라 2개 다 똑같음

    // 3. 디폴트 메서드
    // : 인터페이스에 새 기능을 추가하면서 기존 구현을 포함
    // >> 기본 구현 제공, 오버라이딩(재정의) 가능 // 인터페이스를 쓰긴 써야하는데 재정의를 해야 할 때 사용. 잘 안씀
    // default void 메서드명() { 구현부 }
    // default 생략 불가능. 반드시 작성
    // 생략시 구현부가 존재할 수 없다는 오류 발생
    ...
    // 4. 정적 메서드
    // : 인터페이스에서 직접 구현
    // >> 인터페이스 이름으로 직접 호출, 오버라아딩 불가능)객체 인스턴스 없이 사용 가능
...
class Card implements PlayingCard {
    ...
    // 인터페이스 안에 추상 메서드를 구현한 클래스 메서드는 public 지정
    ...
    // 디폴트 메서드 재정의
    // : 인터페이스 내의 디폴트 메서드 재정의시 public 설정
    @Override
