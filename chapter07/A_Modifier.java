...
: 클래스, 메서드, 변수의 접근 범위를 제어하여 외부로부터
  ...
  >> 접근 제어자가 지정되지 않은 경우 default(기본값)으로 지정됨
...
// 1) private
    ...
    class NotProtectedClass {
        NotProtectedClass() {
            ...
            // Cannot resolve symbol 'otherMethod'
        }
    }
     */

    // 4) public
    // : 어떤 클래스든 어떤 패키지 내에 있든 접근 가능 (인스턴스화는 해야함. 객체로)
...
public class A_Modifier {
    ...
    public static void main(String[] args) {
        ...
        ProtectedClass protectedClass = new ProtectedClass();   // 다른 패키지에 있는 부모 클래스의 필드
                                                                // 다른 패키지에 있는 부모 클래스의 메서드
