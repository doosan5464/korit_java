...
: 한 클래스 내에서 같은 이름의 "메서드"를 여러 개 정의하는 것
    ...
    : 작업의 본질이 동일한 메서드를 하나의 이름으로 그룹화 가능
        -> 코드의 재사용성, 가독성 향상
    : 컴파일 시 타입 체크 강화
        -> 올바른 매개변수를 가진 메서드를 자동으로 선택해줌


    3. 오버로딩의 단점
    : 중복적으로 의미없이 생성하는 경우, 실질적으로 필요한 구현 메서드 찾기에 어려워짐(복잡성)
 */
...
//        return c + d;
//    }
//    Error -> 실제 값 차이 여부와 반환타입 차이 여부는 오버로딩과 관련 X
...
public class Overloading01 {
    public static void main(String[] args) {
        Calc calc = new Calc();
        System.out.println(calc.add(10.0,30)); // 40
