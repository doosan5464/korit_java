...
/*
java.lang 패키지
: 자바 프로그램의 가장 기본적이 클래스들을 포함
- 별도의 import 없이 사용 가능

1. Object 클래스
: 자바에서 모든 클래스의 최상위 클래스
- 모든 클래스는 Object 클래스를 상속받음 (extends Object 가 항상 생략되어 있음)
...
public class JavaLang {
    public static void main(String[] args) {
        ...
        System.out.println(Math.max(100, 200)); // 최대값 .max()
        System.out.println(Math.min(100, 200)); // 최소값 .max()
        System.out.println(Math.sqrt(16));  // 제곱근 .sqrt()
        System.out.println(Math.pow(2, 3)); // 제곱 .pow(a, b) : a^b
        System.out.println(Math.random());  // 랜덤 .random() : 무작위 난수 생성 / 0.0 이상 1.0 미만의 난수

        System.out.println("String 클래스 =============");
        String message = "Hello Christmas!";

        System.out.println(message.length());   // 문자열(공백 포함)의 길이
        ...
        System.out.println(message);        // Hello Christmas!     -> String 은 불변(immutable) 데이터

        // indexOf(String str)
        // : 문자열에서 특정 문자열의 첫 번째 위치의 인덱스를 반환
