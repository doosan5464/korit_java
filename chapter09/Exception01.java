...
- Throwable : 모든 예외와 오류의 최상위 클래스
    ...
    >> 예외 처리를 통해 개발자가 처리할 수 있는 문제들을 정의
        ...
        cf) 컴파일 시점 VS 런타임 시점
            - 컴파일 : 소스 코드를 기계어로 번역하는 단계,
                      실행되지 않고 코드를 분석하고 변환하는 과정에만 국한
                      자바에서는 .java 파일이 컴파일되어 .class 파일(byte코드)로 생성

            - 런타임 : 컴파일된 프로그램이 실행되는 단계,
                      프로그램이 실제로 메모리에 로드되고 CPU에서 실행됨
                      컴파일 시점에서 잡히지 않은 논리적 오류나 컴파일 예외 발생 가능성이 존재
...
public class Exception01 {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println(e.getMessage()); // 예외 : by zero
